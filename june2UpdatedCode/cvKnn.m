...
if K == 1, % sort distances only if K>1
    ...
    % K closest
    KK = min(K, round(.5*size(IndexProto,1)));
    IndexProto = IndexProto(1:KK,:);
    ...
    %@EDITED on March 8
    closestdists = sorted(1,:);
    Class(closestdists>.9*max(closestdists)) = -1; 
    %more unknowns if multiplier lowered
    %@EDIT ends
    Rank = []; 
end